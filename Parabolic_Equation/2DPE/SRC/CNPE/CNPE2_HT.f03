PROGRAM CNPE2_HT
 ...
 ALLOCATE(LIN(0:M),X(M),NN(M),TERR(M),TERR1(M),TERR2(M),SLOPX(M))
 LIN(0)  = 0
 ...
 !-----------------------------------------------------------
 !Output
 OPEN(UNIT=11,FILE="CNPE2_TL.dat")
 OPEN(UNIT=12,FILE="CNPE2_FIELD.dat")
 !OPEN(UNIT=30,FILE="CNPE2_PC.dat")
 !OPEN(UNIT=50,FILE="CNPE2_Topo.dat")
 DO MX = 1,M
  WRITE(11,100) MX*DR, TERR(MX), LPG(MX)
  DO NZ = 1,N
   WRITE(12,100) MX*DR, NZ*DZ+TERR(MX), LP(NZ,MX)
  END DO
  WRITE(12,*)
 END DO
