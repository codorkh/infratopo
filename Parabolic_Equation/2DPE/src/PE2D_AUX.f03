MODULE PE2D_AUX
 ...
 DO J=1,N
  I = DEST_INDEX(J)
  IF ((I.GT.0).AND.(I.LE.M)) DEST(I) = DEST(I)+SRC(J)
 END DO
 ...
 REAL(DP), DIMENSION(:), ALLOCATABLE, INTENT(OUT) :: XUNI, XCNT
 INTEGER :: I,NUM
 ...
 END FUNCTION
 !-----------------------------------------------------------
 SUBROUTINE DENSE2SPARSE(M,N,SM,THRESH)
 IMPLICIT NONE
 TYPE(SPARSE), INTENT(OUT) :: SM
 ...
 END SUBROUTINE
 !-----------------------------------------------------------
! FUNCTION SPARSEMULT(SM,X) RESULT(SMX)
! IMPLICIT NONE
! TYPE(SPARSE), INTENT(IN) :: SM
! COMPLEX(DP), DIMENSION(:), INTENT(IN) :: X
! COMPLEX(DP), DIMENSION(SIZE(X)), INTENT(OUT) :: SMX
! IF (.NOT.(SM%N.EQ.SIZE(X)))
!  STOP "[SPARSEMULT] ERROR : X and Sparse SM should be same size "
! END IF
! B=0.0_DP
